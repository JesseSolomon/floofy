...
}throw`Floofy Exception: location handler must be of type function, got ${typeof d}`;},get:(f,b)=>{if("string"===typeof b){for(let d in a.page_register){let l=b.match(new RegExp(d));if(l)return e=>{if(e&&"object"!==typeof e)throw`Floofy Exception: location state must be of type object, got ${typeof e}`;{let c={};for(let k=0;k<a.page_register[d].capture_groups.length;k++)c[a.page_register[d].capture_groups[k]]=decodeURIComponent(l[k+1]);let h;e={...null!=(h=history.state)?h:{},...null!=e?e:{},...c};
location.pathname!==b&&history.pushState(e,document.title,b);a.page_register[d].handler(e)}}}return null}throw`Floofy Exception: URL selector must be of type string, got ${typeof b}`;}})});addEventListener("DOMContentLoaded",()=>{let f=location.f[location.pathname];f&&f({});addEventListener("popstate",()=>{let b=location.f[location.pathname];b&&b({})})})})(floofy||(floofy={}));
